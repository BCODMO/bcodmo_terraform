resource "aws_ecr_repository" "laminar_worker" {
  name                 = "laminar_worker_${var.environment[terraform.workspace]}"
  image_tag_mutability = "MUTABLE"

}

resource "aws_cloudwatch_log_group" "laminar_worker" {
  name              = "/ecs/laminar_worker_${var.environment[terraform.workspace]}"
  retention_in_days = "7"

}

resource "aws_ecs_task_definition" "laminar_worker" {
  family                = "laminar_worker_${var.environment[terraform.workspace]}"
...
EOF
  ...
  tags = {
    name = var.environment[terraform.workspace] == "default" ? "latest" : var.laminar_version
  }
...
resource "aws_cloudwatch_metric_alarm" "laminar_worker_cpu_low" {
  count               = 1
  alarm_name          = "laminar_worker_cpu_low_${var.environment[terraform.workspace]}"
...
resource "aws_cloudwatch_metric_alarm" "laminar_worker_cpu_high" {
  count               = 1
  alarm_name          = "laminar_worker_cpu_high_${var.environment[terraform.workspace]}"
  alarm_description   = "Managed by Terraform"
