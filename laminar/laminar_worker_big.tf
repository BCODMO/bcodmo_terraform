...
[
    {
        ...
        "logConfiguration": {
            "logDriver": "awslogs",
            "options": {
                "awslogs-group": "${aws_cloudwatch_log_group.laminar_worker_big.name}",
...
resource "aws_ecs_service" "laminar_worker_big" {
  ...
  task_definition      = aws_ecs_task_definition.laminar_worker_big.arn
  desired_count        = 0
...
resource "aws_appautoscaling_target" "laminar_worker_big_target" {
  max_capacity       = 4
  min_capacity       = 1
  resource_id        = "service/${aws_ecs_cluster.laminar.name}/${aws_ecs_service.laminar_worker_big.name}"
...
resource "aws_appautoscaling_policy" "laminar_worker_big_cpu_low" {
  ...
  service_namespace  = aws_appautoscaling_target.laminar_worker_big_target.service_namespace
  ...
  depends_on = [aws_appautoscaling_target.laminar_worker_big_target]
}
...
resource "aws_cloudwatch_metric_alarm" "laminar_worker_big_cpu_low" {
  ...
  dimensions = {
    ...
    ServiceName = aws_ecs_service.laminar_worker_big.name
  }
}
...
output "laminar_worker_big_name" {
  value       = aws_ecs_service.laminar_worker_big.name
  description = "The name of the laminar worker big service"
}
